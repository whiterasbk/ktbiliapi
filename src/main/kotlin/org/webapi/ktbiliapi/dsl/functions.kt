 ...
 * 具体使用方法:
 * ```kotlin
 * val wbiCtx = createWbiSignContext()
 ...
 *
 * 和 [createWbiSignContext] 使用方式基本一致
 *
 ...
 * val loginCtx = createWbQRCodeLoginContext(userCtx)
 * val responseBody = wbiCtx {
 *      getDashResponseBodies("bvid").first()
 * }
 * ```
 ...
 */
fun createWebQRCodeLoginContext(userCtx: QRUserContext): StateDSLContext {
    val factory = SequenceOperateBGMHandlerFactory()
...
/**
 * 创建附带的 web 二维码登录信息, 并使用 wbi 签名的 dsl 上下文对象
 * @see [createWbiSignContext]
 * @see [createWebQRCodeLoginContext]
 */
fun createWebQRCodeLoginAndSignViaWbiContext(userCtx: QRUserContext): StateDSLContext {
    val factory = SequenceOperateBGMHandlerFactory()
    factory.addOperateParams { signViaWbiCached(it) }
    factory.addOperateHeaders { addQRCodeUserCookies(userCtx, it) }
    return StateDSLContext(factory.getHandler())
